...
function ConditionOptions() {
    ...
    const handleChangeRoom = (event, name) => { // 修改房屋格局
        ...
        if (event.target.value >= 0) {
            dispatch(SetSearch("setRoom", {...SearchData.room, [name]: parseInt(event.target.value)}));
        }
    ...
    return (
        <Grid container columns={16} spacing={2}>
            ...
            <Grid item md={16} sx={{display: "flex"}}>
                ...
                <Box sx={{display: "flex"}}>
                    <TextField value={SearchData.room["房間"]} onChange={(e) => handleChangeRoom(e, "房間")} size="small"
                        type="number" inputProps={{inputMode: "numeric", pattern: "[0-9]*"}}
                        InputLabelProps={{shrink: true}} sx={{width: "4rem"}}
                        disabled={SearchData.roomType !== null}/>
                    <Typography>房</Typography>
                    <TextField value={SearchData.room["衛浴"]} onChange={(e) => handleChangeRoom(e, "衛浴")} size="small"
                        type="number" inputProps={{inputMode: "numeric", pattern: "[0-9]*"}}
                        InputLabelProps={{shrink: true}} sx={{width: "4rem"}}
                        disabled={SearchData.roomType !== null}/>
                    <Typography>衛</Typography>
                    <TextField value={SearchData.room["廳數"]} onChange={(e) => handleChangeRoom(e, "廳數")} size="small"
                        type="number" inputProps={{inputMode: "numeric", pattern: "[0-9]*"}}
                        InputLabelProps={{shrink: true}} sx={{width: "4rem"}}
                        disabled={SearchData.roomType !== null}/>
